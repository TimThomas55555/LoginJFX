...
public class Controlador {//controlado del login, es el que pide nombre de usuario y contraseña y pasa las imagenes. Este solo se encarga de pasar las imagenes.
	...
	public void abrirHomepage() {//el @FXML es que está referenciado en el Scenne Builder y eso significa que esta en el boton On Action para que al pulsar el boton login, vaya a Homepage
		...
		try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("Homepage.fxml"));//carga la vista que deseo, en este caso homepage
            AnchorPane page = (AnchorPane) loader.load();//Cargame otro AnchorPane
            Stage sendStage = new Stage();    
            Scene scene = new Scene(page);
            sendStage.initStyle(StageStyle.UNDECORATED);//Esto es para que no me salgan los bordes de la ventana
